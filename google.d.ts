interface Window {
  google?: {
    accounts: {
      id: {
        initialize: (input: Google.IdConfiguration) => void;
        prompt: (momentListener?: Google.MomentListener) => void;
        renderButton: (parent: HTMLElement, options: Google.GsiButtonConfiguration) => void;
      ...
      oauth2: {
        initTokenClient: (config: Google.TokenClientConfig) => Google.InitTokenClient;
        initCodeClient: (config: Google.CodeClientConfig) => Google.InitCodeClient;
        hasGrantedAnyScope: (tokenResponse: Google.TokenResponse, firstScope: string, ...restScopes: string[]) => boolean;
        hasGrantedAllScopes: (tokenResponse: Google.TokenResponse, firstScope: string, ...restScopes: string[]) => boolean;
        revoke: (accessToken: string, done?: () => void) => void;
      };
    };
  };
...
declare namespace Google {
  ...
  type ErrorCode = "invalid_request" | "access_denied" | "unauthorized_client" | "unsupported_response_type" | "invalid_scope" | "server_error" | "temporarily_unavailable";

  interface InitTokenClient {
    requestAccessToken: (overridableClientConfig?: Google.OverridableTokenClientConfig) => void;
  }

  interface InitCodeClient {
    requestCode: () => void;
  }
  ...
  type MomentListener = (promptMomentNotification: PromptMomentNotification) => void;
}
